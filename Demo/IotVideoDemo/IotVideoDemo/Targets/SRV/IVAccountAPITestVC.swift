...
class IVAccountAPITestVC: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let type = IoTVideo.sharedInstance.options?[IVOptionKey.hostType] as? String {
            self.title = type == "0" ? "账户（测试服）" : "账户（正式服）"
        }
        getDataArray()
        
        let header = UIView(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 200))
        let userName = UILabel(frame: CGRect(x: 18, y: 18, width: view.frame.width - 36, height: 24))
        userName.text = "用户名：\(UserDefaults.standard.string(forKey: demo_userName)!)"
        let accessId = UILabel(frame: CGRect(x: 18, y: 60, width: view.frame.width - 36, height: 20))
        accessId.text = "accessId：\(IoTVideo.sharedInstance.accessId)"
        let terminalld = UILabel(frame: CGRect(x: 18, y: 100, width: view.frame.width - 36, height: 20))
        terminalld.text = "terminalld: \(IoTVideo.sharedInstance.terminalId)"
        let acceessToken = UILabel(frame: CGRect(x: 18, y: 140, width: view.frame.width - 36, height: 60))
        acceessToken.text = "acceessToken: \(IoTVideo.sharedInstance.accessToken)"
        header.addSubview(userName)
        header.addSubview(accessId)
        header.addSubview(terminalld)
        header.addSubview(acceessToken)
