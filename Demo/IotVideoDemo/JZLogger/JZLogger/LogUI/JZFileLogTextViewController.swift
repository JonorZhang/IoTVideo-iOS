...
class JZFileLogTextViewController: UIViewController, JZFileLoggerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if case .url(let url)? = content, JZFileLogger.shared.currLogFileURL == url {
            navigationItem.rightBarButtonItems = [
                UIBarButtonItem(title: "分享", style: .plain, target: self, action: #selector(sharing)),
                UIBarButtonItem(title: "悬浮", style: .plain, target: self, action: #selector(suspend))
            ]
        } else {
    ...
    @objc func suspend() {
        ...
        IVLogAssistant.shared.suspend(content: content)
    }
